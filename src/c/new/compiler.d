...
pop(cl_object *l) {
	...
	if (ATOM(list))
		FEerror("Error parsing special form",0);
	head = CAR(list);
	*l = CDR(list);
	return head;
}
...
asm_op2(register int code, register cl_fixnum n) {
	...
	if (n < -MAX_OPARG || MAX_OPARG < n)
		FEerror("Argument to bytecode is too large", 0);
	else
		asm1(new_op);
}
...
asm_insert(cl_fixnum where, cl_object op) {
	...
	if (where > end)
		FEerror("asm1_insert: position out of range", 0);
...
asm_at(register cl_index where, register cl_object what) {
	if (where > bytecodes->vector.fillp)
		FEerror("Internal error at asm_at()",0);
...
asm_complete(register int op, register cl_index original) {
	...
	if (code != MAKE_FIXNUM(op))
		FEerror("Non matching codes in ASM-COMPLETE2", 0);
	else if (delta < -MAX_OPARG || delta > MAX_OPARG)
		FEerror("Too large jump", 0);
	else
...
{
	if (!SYMBOLP(var))
		FEerror("The object ~A is not a valid variable name", 1, var);
...
compile_setq(int op, cl_object var)
{
	...
	if (!SYMBOLP(var))
		FEerror("SETQ: ~A is not a symbol", 1, var);
	ndx = lex_var_sch(var);
	if (!Null(ndx) && CDR(ndx) != Sspecial)
		asm_op(op); /* Lexical variable */
	else if (var->symbol.stype == stp_constant)
		FEerror("SETQ: Cannot change the value of the constant ~A", 1,var);
	else if (op == OP_SETQ)
...
static void
bind_tag(cl_object tag)
{
	CDR(lex_env) = CONS(list(3, tag, Stag, Cnil), CDR(lex_env));
}

static bool
reference_tag(cl_object tag)
{
	cl_object l;
	bool non_local = FALSE;

	for (l = CDR(lex_env); !endp(l); l = CDR(l)) {
		cl_object record = CAR(l);
		if (ATOM(record))
			non_local = TRUE;
		else if (CAR(record) == Ct && CDR(record) == Cnil)
			non_local = TRUE;
		else if (eql(CAR(record), tag) && CADR(record) == Stag) {
			cl_object mark;
			if (non_local)
				mark = Ct;
			else
				mark = MAKE_FIXNUM(current_pc());
			CADDR(record) = nconc(CADDR(record), CONS(mark, Cnil));
			return non_local;
		}
	}
	FEerror("Reference to unknown tag ~S", 1, tag);
}
...
c_and(cl_object args) {
	if (Null(args)) {
		...
		FEerror("Wrong type of argument to AND ~S", 1, args);
	} else {
...
c_block(cl_object body) {
	...
	if (!SYMBOLP(name))
		FEerror("Not a valid BLOCK name: ~S", 1, name);
	asm1(name);
	compile_body(body);
	asm_op(OP_EXIT);
...
c_call(cl_object args, bool push) {
	...
	if (ATOM(name)) {
		...
		if (aux == OBJNULL)
			FEerror("Invalid function name ~S", 1, name);
		asm_op2(push? OP_PCALL : OP_CALL, nargs);
		asm1(aux);
	}
}
...
c_do_doa(int op, cl_object args) {
	...
	for (l=bindings; !endp(l); ) {
		...
		if (ATOM(aux)) {
			...
			if (!Null(aux))
				FEerror("Not a valid argument to LET ~S.", 1,
					args);
		}
		if (!SYMBOLP(var))
			FEerror("Cannot bind ~S.", 1, var);
		if (op == OP_PBIND) {
...
c_dolist_dotimes(int op, cl_object args) {
	...
	if (!SYMBOLP(var))
		FEerror("Cannot bind to ~S", 1, var);
	...
	if (CDR(head) != Cnil)
		FEerror("Too many arguments to output form of DOLIST", 0);
	if (Null(head))
		compile_body(Cnil);
	else {
		compile_setq(OP_SETQ, var);
...
c_function(cl_object args) {
	...
	if (!endp(args))
		FEerror("Too many arguments to FUNCTION", 0);
	if (SYMBOLP(function)) {
		asm_op(OP_FUNCTION);
		asm1(function);
		...
		FEerror("No a valid argument to FUNCTION ~S", 1, function);
}
...
c_go(cl_object args) {
	...
	if (!Null(args))
		FEerror("Too many arguments to GO",0);
	if (!reference_tag(tag))
		asm_op(OP_JMP);		/* Local tag */
	else {
		asm_op(OP_GO);		/* Tagbody out of closure */
		asm1(tag);
	}
}
...
c_if(cl_object form) {
	...
	if (!Null(form))
		FEerror("Too many arguments to IF form", 0);
}
...
c_multiple_value_setq(cl_object args) {
	...
	for (orig_vars = reverse(pop(&args)); !endp(orig_vars); ) {
		...
		if (!SYMBOLP(v))
			FEerror("Cannot bind ~S", 1, v);
		v = macro_expand1(v, CDR(lex_env));
		if (!SYMBOLP(v)) {
			aux = v;
			v = Lgensym(0);
	...
	compile_form(pop(&args), FALSE);
	if (args != Cnil)
		FEerror("Too many arguments to MULTIPLE-VALUE-SETQ",0);
	if (nvars == 0)
		/* No variables */
		return;
	...
	while (nvars--) {
		...
		if (!SYMBOLP(var))
			FEerror("MULTIPLE-VALUE-SETQ: ~A is not a symbol", 1, var);
		ndx = lex_var_sch(var);
		if (!Null(ndx) && CDR(ndx) != Sspecial)
			asm1(var); /* Lexical variable */
		else if (var->symbol.stype == stp_constant)
			FEerror("MULTIPLE-VALUE-SETQ: Cannot change the value of the constant ~A", 1,var);
		else {
			asm1(MAKE_FIXNUM(1));
			asm1(var);
		}
...
c_nth_value(cl_object args) {
	...
	if (args != Cnil)
		FEerror("Too many arguments to NTH-VALUE",0);
	asm_op(OP_NTHVAL);
}
...
c_psetq(cl_object old_args) {
	...
	*/
	while (!endp(old_args)) {
		...
		if (!SYMBOLP(var))
			FEerror("Cannot assign to ~a", 1, var);
		var = macro_expand1(var, CDR(lex_env));
		if (!SYMBOLP(var))
			use_psetf = TRUE;
...
c_return(cl_object stmt) {
	...
	asm1(Cnil);
	if (stmt != Cnil)
		FEerror("Too many arguments to RETURN", 0);
}
...
c_return_from(cl_object stmt) {
	...
	asm_op(OP_RETURN);
	if (!SYMBOLP(name))
		FEerror("Not a valid return tag ~S", 1, name);
	asm1(name);
	if (stmt != Cnil)
		FEerror("Too many arguments to RETURN-FROM", 0);
}
...
c_symbol_macrolet(cl_object args)
{
	...
	for (; !endp(def_list); ) {
		...
		if (special_variablep(name, specials))
			FEerror("Symbol ~A cannot be declared special and appear in a SYMBOL-MACROLET", 1, name);
...
{
	int i, nt;
	cl_fixnum label0, labelz;
	cl_object body, tag_list;
	cl_object lex_old = lex_env;
	lex_copy();

	/* count and register the tags */
	for (nt = 0, tag_list = Cnil, body = args; !endp(body); body = CDR(body)) {
		cl_object form = CAR(body);
		int type = type_of(form);
		if (type == t_symbol || type == t_fixnum || type == t_bignum) {
			tag_list = CONS(list(3, form, Stag, Cnil), tag_list);
			nt += 1;
		}
	}
	if (nt == 0) {
	...
	label0 = current_pc();
	CDR(lex_env) = nconc(nreverse(tag_list), CDR(lex_env));

	/*
	 * We compile the body, storing the address of each label.
	 */
	for (tag_list = CDR(lex_env), body = args; !endp(body); body = CDR(body)) {
		cl_object form = CAR(body);
		int type = type_of(form);
		if (type == t_symbol || type == t_fixnum || type == t_bignum) {
			/* Each label points to a place in the bytecodes
			   stack. This point is registered at the beginning of
			   the tag list. */
			cl_object pc = MAKE_FIXNUM(current_pc());
			cl_object tag = CAR(tag_list);
			cl_object relocation = CADDR(tag);
			CADDR(tag) = CONS(pc, relocation);
			tag_list = CDR(tag_list);
		} else if (CONSP(form)) {
			/* Since the output of all forms is ignored, we
			   need only compile forms which are not atoms */
			compile_form(form, FALSE);
		}
	}
	/*
	 * Each (GO ...) form leads to either a local jump, or a jump
	 * out of a closure. In this loop we fix the destination of those
	 * local jumps...
	 */
	for (i = 0, tag_list = Cnil; nt; nt--, CDR(lex_env) = CDDR(lex_env)) {
		cl_object tag = CADR(lex_env);
		cl_object tag_name = CAR(tag);
		cl_object relocation = CADDR(tag);
		cl_fixnum pc = fix(pop(&relocation));
		while (!endp(relocation)) {
			cl_object jump = pop(&relocation);
			if (FIXNUMP(jump)) {
				cl_fixnum pc_orig = fix(jump);
				asm_at(pc_orig, make_op2(OP_JMP, pc - pc_orig));
				continue;
			}
			tag_list = nconc(tag_list,
					 list(2, tag_name, MAKE_FIXNUM(pc)));
			i++;
		}
	}
	/*
	 * ...and in this loop we keep a record of the tags that were
	 * referenced by nonlocal jumps. This implies inserting an OP_TAGBODY
	 * operand with its relocation table at LABEL0.
	 */
	if (!i)
		compile_body(Cnil);
	else {
		cl_fixnum delta = 1 + 2*i;
		cl_fixnum label;
		asm_op(OP_EXIT);
		asm_insert(label0, make_op2(OP_TAGBODY, i));
		while (!endp(tag_list)) {
			cl_object name = pop(&tag_list);
			cl_fixnum pc = fix(pop(&tag_list)) + delta;
			asm_insert(label0, name); label0++;
			asm_insert(label0, MAKE_FIXNUM(pc - label0)); label0++;
		}
	}
}
...
c_throw(cl_object stmt) {
	...
	if (stmt != Cnil)
		FEerror("Too many argumnents to THROW",0);
...
compile_form(cl_object stmt, bool push) {
 ...
 LIST:
	...
	if (function == Squote) {
		stmt = CDR(stmt);
		if (CDR(stmt) != Cnil)
			FEerror("Too many arguments to QUOTE",0);
		stmt = CAR(stmt);
		goto QUOTED;
	}
	 ...
	 */
	{
		cl_object new_stmt = macro_expand1(stmt, CDR(lex_env));
		if (new_stmt != stmt){
			stmt = new_stmt;
			goto BEGIN;
		}
	...
	if (function->symbol.isform)
		FEerror("Found no macroexpander for special form ~S", 1, function);
 ORDINARY_CALL:
	/*
...
#define push_var(v, list) \
	...
	if (v->symbol.stype == stp_constant) \
		FEerror("~S is not a variable.", 1, v); \
	push(v, list);
...
@
	...
	for (; !endp(body); body = CDR(body)) {
	  ...
	  for (decls = CDR(form); !endp(decls); decls = CDR(decls)) {
	    ...
	    if (ATOM(sentence))
	      FEerror("Illegal declaration form", 1, form);
...
KEYWORD:
	while (1) {
		...
		if (CONSP(v)) {
			x = v;
			key = CAR(x);
			if (key->symbol.hpack != keyword_package)
				FEerror("~S is not a keyword", 1, key);
			if (endp(CDR(x)) || !endp(CDDR(x)))
				goto ILLEGAL_LAMBDA;
			v = CADR(x);
		} else {
			check_symbol(v);
...
ILLEGAL_LAMBDA:
	FEerror("Illegal lambda list ~S.", 1, CAR(lambda));
@)
...
c_register_var2(register cl_object var, register cl_object *specials)
{
	...
	if (member_eq(var, *specials))
		...
		*specials = CONS(var, *specials);
		...
		FEerror("Cannot bind the constant ~A", 1, var);
	else
		c_register_var(var, FALSE);
}
...
make_lambda(cl_object name, cl_object lambda) {
	...
	/* Mark closure boundary */
	CDR(lex_env) = CONS(CONS(Ct, Cnil), CDR(lex_env));
