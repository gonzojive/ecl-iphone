...
lambda_bind(int narg, cl_object lambda_list, cl_object *args)
{
	...
	if (n != 0 || other_keys) {
	  ...
	  for (; narg; args+=2, narg-=2) {
	    if (!SYMBOLP(args[0]) || args[0]->symbol.hpack != keyword_package)
	      FEerror("Not a keyword ~S.", 1, args[0]);
	    keys = data;
	    ...
	    if (args[0] != SAallow_other_keys)
	      other_found = TRUE;
	    else
	      other_keys = (args[1] == Cnil);
	  FOUND:
	  }
	  if (other_found && !other_keys)
	    FEerror("Unknown keys found", 0);
	...
	if (narg && !other_keys && check_remaining)
	  FEerror("Too many arguments to function ~S", 1,
...
interpret_tagbody(cl_object *vector) {
	cl_index i, ntags = fix(get_oparg(vector[-1]));
	cl_object lex_old = lex_env;
	cl_object id = new_frame_id();
	...
	while (i--) {
		...
		if (var != MAKE_FIXNUM(1))
			...
			if (var->symbol.stype == stp_constant)
				FEerror("Cannot set the constant ~A", 1, var);
			else
				SYM_VAL(var) = value;
		}
	}
...
interpret(cl_object *vector) {
 ...
 BEGIN:
	...
	case OP_RETURN: {
		...
		if (Null(id))
			FEerror("The block name ~S is undefined.", 1, tag);
		return_from(id, tag);
		break;
	}
	case OP_THROW:
