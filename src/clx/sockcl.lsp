     ...
     object elem;		/* 'base-char */
{
   ...
   if (fp == NULL) {
     ...
     vs_push(stream);
     setbuf(fp, stream->sm.sm_buffer = alloc(BUFSIZ));
   }
   vs_reset;
   return(stream);
}
...
(defun open-socket-stream (host display)
  ...
  (let ((fd (konnect-to-server host display)))	; get a file discriptor
    (if (< fd 0)
	NIL
	(let ((stream-in  (konnect-stream host fd 0 'base-char))	; input
	      (stream-out (konnect-stream host fd 1 'base-char)))	; output
