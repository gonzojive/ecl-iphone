    ...
    (cond ((null stream))
	  ...
	  ((not (null timeout))
	   (multiple-value-bind (npoll fraction)
	     ...
	     #+ecl (declare (fixnum npoll fraction))
...
#+(or (not ecl clx-ansi-common-lisp) lispm allegro)
...
#+(or ecl (and clx-ansi-common-lisp (not lispm) (not allegro)))
(defun default-keysym-translate (display state object)
  (declare (type display display)
	   (type card16 state)
	   (type t object)
